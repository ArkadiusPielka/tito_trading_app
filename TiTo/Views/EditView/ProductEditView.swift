...
struct ProductEditView: View {
                    ...
                    AsyncImage(url: URL(string: product.imageURL ?? "")) { phase in
                        ...
                        case .empty:
                            Image(systemName: "photo")
                                .resizable()
                                .scaledToFit()                    case .success(let image):
                            if let selectedImage = productViewModel.selectedImageData, let image = UIImage(data: selectedImage) {
                                Image(uiImage: image)
                                    .resizable()
                                    .scaledToFit()                                .frame(maxWidth: CGFloat.maxScreenWidth, maxHeight: 300)
                            } else {
                                image
                                    .resizable()
                                    .scaledToFit()                                .frame(maxWidth: CGFloat.maxScreenWidth, maxHeight: 300)
    ...
    func updateProduct() {
        ...
        if updatedProduct.title != currentProduct.title ||
            updatedProduct.category != currentProduct.category ||
            updatedProduct.condition != currentProduct.condition ||
            updatedProduct.shipment != currentProduct.shipment ||
            updatedProduct.optional != currentProduct.optional ||
            updatedProduct.description != currentProduct.description ||
            updatedProduct.material != currentProduct.material ||
            updatedProduct.price != currentProduct.price ||
            updatedProduct.priceType != currentProduct.priceType ||
...
#Preview {
    ProductEditView(
        product: FireProduct(
            ...
            description: "blablaashgf ouawhuoh guahs ufghwuhfua fshguva ushfoug hasufhu ashdfu gsfdubasuf gasbdfasd fkjbasbfdas bfhasbfhba sdfbashkbfkas bfhbashfdbhas dbvasbvbw sbvhs dabvh sdbahjf", advertismentType: "Ich biete", price: "20", priceType: "VB",
